...
namespace M3D_ISICG
{
	const std::string Scene::_shaderFolder = "src/scene/shaders/";
	...
	bool Scene::init()
	{
		...
		_scene.load("sponza", "assets/sponza.obj");
	...
	void Scene::animate(const float p_deltaTime) {}
	...
	void Scene::render()
	{
		...
		glProgramUniformMatrix4fv(_program, _uMMatrix, 1, false, glm::value_ptr(_scene._transformation));
		glProgramUniformMatrix4fv(_program, _uMVPMatrixLoc, 1, false, glm::value_ptr(_camera->getProjectionMatrix() * _camera->getViewMatrix() * _scene._transformation));
		Vec3f posCam = _camera->getPosition();
	...
	void Scene::handleEvents(const SDL_Event& p_event)
	{
	...
	void Scene::resize(const int p_width, const int p_height)
	{
	...
	bool Scene::_initProgram()
	{
		...
		_uMMatrix = glGetUniformLocation(_program, "uMMatrix");
