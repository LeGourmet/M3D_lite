...
namespace M3D_ISICG
{
	...
	Texture TriangleMeshModel::_loadTexture( const aiString & p_path, const std::string & p_type )
	{
		...
		if ( image.load( fullPath ) )
		{
			...
			unsigned int lvMipMap = std::floor(std::log2(std::max(image._width, image._height))) + 1;
			glTextureStorage2D( texture._id, lvMipMap, internalFormat, image._width, image._height );
