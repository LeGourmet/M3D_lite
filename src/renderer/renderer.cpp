...
namespace M3D
{
	...
	Renderer::~Renderer() {
		...
		for (std::pair<Mesh*, MeshOGL*> pair : _meshes) delete pair.second;
		for (std::pair<Texture*, TextureOGL*> pair : _textures) delete pair.second;
	}

	void Renderer::resize(const uint p_width, const uint p_height) {
	...
	void Renderer::createMesh(Mesh* p_mesh) { _meshes.insert(std::pair<Mesh*, MeshOGL*>(p_mesh, new MeshOGL(p_mesh))); }
	void Renderer::createTexture(Texture* p_texture) { _textures.insert(std::pair<Texture*, TextureOGL*>(p_texture, new TextureOGL(p_texture))); }

	void Renderer::addInstanceMesh(Mesh* p_mesh, const Mat4f& p_M_matrix, const Mat4f& p_V_matrix, const Mat4f& p_P_matrix) { _meshes.at(p_mesh)->addInstance(p_M_matrix, p_V_matrix, p_P_matrix); }
	void Renderer::updateInstanceMesh(Mesh* p_mesh, uint p_id, const Mat4f& p_M_matrix, const Mat4f& p_V_matrix, const Mat4f& p_P_matrix) { _meshes.at(p_mesh)->updateTransformMatrix(p_id, p_M_matrix, p_V_matrix, p_P_matrix); }

	void Renderer::deleteMesh(Mesh* p_mesh) { delete _meshes.at(p_mesh); _meshes.erase(_meshes.find(p_mesh)); }
	void Renderer::deleteTexture(Texture* p_texture) { delete _textures.at(p_texture); _textures.erase(_textures.find(p_texture)); }
}