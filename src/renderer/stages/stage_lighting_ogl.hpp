...
namespace M3D
{
	...
	public:
		...
		void execute(uint p_width, uint p_height, std::map<Mesh*, MeshOGL*> p_meshes, std::map<Texture*, TextureOGL*> p_textures, GLuint p_albedoMap, GLuint p_normalMap, GLuint p_metalnessRoughnessMap, GLuint p_emissiveMap, GLuint p_depthMap, GLuint p_rootTransparency, GLuint p_ssboTransparency) {
			// --- clear buffer ---
			...
			for (Light l : Application::getInstance().getSceneManager().getLights())
				for (uint i = 0; i < l.getNumberInstances();i++) {
					...
					if (l.getType() == LIGHT_TYPE::DIRECTIONAL) {
						...
						for (std::pair<Mesh*, MeshOGL*> mesh : p_meshes)
							for (uint j = 0; j < mesh.first->getSubMeshes().size(); j++) {
								SubMesh subMesh = mesh.first->getSubMeshes()[j];
	...
	private:
		...
		uint _shadowMapResolution = 1024;

		ProgramOGL _ShadowPass = ProgramOGL("resources/shaders/shadow/Shadow.vert", "", "resources/shaders/shadow/Shadow.frag");
		ProgramOGL _ShadowCubePass = ProgramOGL("resources/shaders/shadow/CubeShadow.vert", "resources/shaders/shadow/CubeShadow.geom", "resources/shaders/shadow/CubeShadow.frag");

		ProgramOGL _DirectLightingPass = ProgramOGL("resources/shaders/utils/QuadScreen.vert", "", "resources/shaders/lighting/DirectLightingPass.frag");
		ProgramOGL _IndirectLightingPass = ProgramOGL("resources/shaders/utils/QuadScreen.vert", "", "resources/shaders/lighting/IndirectLightingPass.frag");
		ProgramOGL _FinalMixLightingPass = ProgramOGL("resources/shaders/utils/QuadScreen.vert", "", "resources/shaders/lighting/FinalMixLightingPass.frag");
	};
}
