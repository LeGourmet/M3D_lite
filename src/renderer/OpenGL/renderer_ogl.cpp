...
namespace M3D
{
	namespace Renderer
	{
			void RendererOGL::drawFrame(SDL_Window* p_window){
				...
				_shadingPass->execute(_viewport_width,_viewport_height,_geometryPass->getPositionMetalnessMap(),_geometryPass->getNormalRoughnessMap(),_geometryPass->getAlbedoMap(),_emptyVAO);
				_finalPass->execute(_viewport_width,_viewport_height,_gamma,_geometryPass->getAmbientMap(),_emptyVAO);
