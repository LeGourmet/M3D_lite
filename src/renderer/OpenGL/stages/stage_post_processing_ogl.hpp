	...
	namespace Renderer
	{
		...
		public:
			...
			StagePostProcessingOGL() {
				...
				glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB16, width, height, 0, GL_RGB, GL_UNSIGNED_SHORT, image);
			...
			void resize(int p_width, int p_height) {
				...
				for(unsigned int i=1; i<lvMipMap+1 ;i++){
					...
					resizeColorMap(GL_RGB16F, GL_RGB, GL_FLOAT, (int)_bloomMapsDims[i].x, (int)_bloomMapsDims[i].y, _bloomMaps[i]);
				}
